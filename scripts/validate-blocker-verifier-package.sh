...
mkdir -p /tmp/lp
if [ ! -x /tmp/lp/ledger-preflight-0.1.0/ledger-preflight ]; then
  tar -xzf /dist/ledger-preflight-0.1.0-linux-x86_64.tar.gz -C /tmp/lp
fi
...
drive(){
  set -e
  wait_new 1 '> Continue'
  tr '\000' '\n' < /proc/"$(cat /tmp/lp/blocker-verifier.pid)"/cmdline > "$out.command.txt"
  at=$(position);printf '\r'
  wait_new "$at" "$first"
  if [ "$variant" = blocked-no-tvu ] || [ "$variant" = blocked-tvu ]; then
...
for variant in blocked-no-tvu blocked-tvu compatible ready; do
  if [ "$variant" = compatible ] || [ "$variant" = ready ]; then source=/dist/synthetic/blocker-verifier/compatible; else source=/dist/synthetic/blocker-verifier/blocked; fi
  base=/tmp/blocker-verifier-$variant
  mkdir -p "$base"
  cp -a "$source/node" "$source/kit" "$source/host.json" "$source/classpath.txt" "$source/tvu.log" "$source/errors.zip" "$base/"
  chmod -R a-w "$base/node" "$base/kit"
  test ! -w "$base/node"
  test ! -w "$base/kit"
  out=/output/blocker-verifier-$variant-$VERSION_ID
  file=$out.ansi
  ...
  if [ "$variant" = compatible ]; then
    ...
    expected_exit=2;expected_status=BLOCKED;schema=ExampleSchema
  fi
  if [ "$variant" = blocked-tvu ] || [ "$variant" = ready ]; then command="$command --tvu-results $base/tvu.log --tvu-results $base/errors.zip"; fi
  rm -f "$file"
  set +e
  drive | env TERM=xterm-256color LANG=C.UTF-8 LC_ALL=C.UTF-8 timeout --preserve-status --kill-after=3s 180s script -q -e -f -c "stty cols 96 rows 56; echo \$\$ > /tmp/lp/blocker-verifier.pid; exec $command" "$file" > /dev/null
  code=$?
  set -e
  printf '%s\n' "$code" > "$out.exit.txt"
  ...
  if [ "$variant" = compatible ] || [ "$variant" = ready ]; then
    grep -qF "$expected_status" "$file"
    if grep -qF 'View compatibility evidence' "$file"; then exit 1; fi
    grep -qF 'View schema evidence' "$file"
  else
