...
assert unchanged['status'] == 'PASS' and unchanged['importOnlySurface'] is True
assert unchanged['environmentReviewed'] is True and unchanged['schemaEvidenceReviewed'] is True
...
verified('P', 'Environment values and original source remain unchanged', test('SimpleResultActionsTest', 'environmentUsesSelectedRuntimeAndHidesHistoricalDetails'), 'release-ux-review.json', 'screenshots/19-execution-context-environment.png', '12 packaged Environment screens')
verified('Q', 'Schema evidence remains explicit and distinguishes imported proof from static configuration', test(ux, 'schemaWordingAndStructureRemainUnchangedWithContextFindings'), 'release-ux-review.json', 'screenshots/22-execution-context-schema.png')
