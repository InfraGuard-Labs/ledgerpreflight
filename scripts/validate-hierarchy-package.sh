...
drive(){
  set -e
  wait_new 1 '> Continue'
  tr '\000' '\n' < /proc/"$(cat /tmp/run-a-package/hierarchy.pid)"/cmdline > "$out.command.txt"
...
for variant in blocked-no-tvu blocked-tvu compatible; do
  if [ "$variant" = compatible ]; then source=/dist/synthetic/hierarchy/compatible; else source=/dist/synthetic/hierarchy/blocked; fi
  base=/tmp/hierarchy-$variant
  mkdir -p "$base"
  ...
  test ! -w "$base/node"
  test ! -w "$base/kit"
  out=/output/hierarchy-$variant-$VERSION_ID
  file=$out.ansi
