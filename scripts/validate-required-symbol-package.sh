...
drive(){
  ...
  wait_new "$at" '> View compatibility evidence'
  at=$(position);printf '\r'
  wait_new "$at" 'Method missing'
  wait_new "$at" '> Back'
  at=$(position);printf '\r'
  wait_new "$at" '> View compatibility evidence'
  # Static compatibility + schema: export is the third action, support is the fourth.
...
if grep -Eq 'OutOfMemoryError|Exception in thread|Compatibility analysis incomplete|View technical evidence|View full technical report|View TVU evidence|TVU instructions|currentInventory|LP-API|LP-INPUT|LP-ANALYSIS|Ljava/|[0-9]+ warnings|650 processed|201 failed|Transactions processed' "$file"; then
