...
for locale in C C.UTF-8; do
  ...
  ! grep -q 'Discovery confidence\|Other JARs\|❯\|□\|■\|▶' "$file"
done
...
! grep -qF "$hide" /output/discovery-terminal-$VERSION_ID-plain.txt
set +e
...
set -e
test "$code" = 1
! grep -qF "$(printf '\033')" /output/discovery-terminal-$VERSION_ID-noninteractive.txt
