"""Capture real packaged verifier and blocker-readiness CLI sessions in Ubuntu PTYs.
...
WORK = pathlib.Path('/work/blocker-verifier')
SHOTS = ROOT / 'screenshots'
TRANSCRIPTS = ROOT / 'terminal-transcripts'
OUTPUT = ROOT / 'blocker-verifier-artifacts'
...
class Terminal:
    def __init__(self, variant):
        ...
        self.name = 'blocker-verifier-' + variant
        ...
        if variant in ('blocked-tvu', 'ready'):
    ...
    def finish(self, expected):
        ...
        while time.monotonic() < end:
            ...
            if done:
                ...
                if self.variant not in ('blocked-tvu', 'ready'):
...
spec = importlib.util.spec_from_file_location('blocker-verifier_checks', pathlib.Path(__file__).with_name('validate-blocker-verifier.py'))
checks = importlib.util.module_from_spec(spec)
spec.loader.exec_module(checks)

for variant in checks.VARIANTS:
    source = ROOT / 'synthetic/blocker-verifier' / ('compatible' if variant in ('compatible', 'ready') else 'blocked')
    ...
    if variant == 'blocked-no-tvu':
        terminal.capture('39-blocker-verifier-environment.png', 'Unchanged Environment with generated verifier fixture')
    terminal.action('Continue')
    terminal.menu()
    checks.result_text(terminal.display(), variant)
    if variant in ('compatible', 'ready'):
        assert checks.STATUS[variant] in terminal.display() and 'View compatibility evidence' not in terminal.display()
        assert 'View schema evidence' in terminal.display()
        terminal.capture('48-blocker-verifier-ready-for-tvu.png' if variant == 'compatible' else '49-blocker-verifier-ready-to-upgrade.png',
                         'Static configuration and compatibility proven; TVU remains required' if variant == 'compatible' else 'Static checks and supplied complete TVU evidence passed')
    else:
        ...
        if variant == 'blocked-no-tvu':
            terminal.capture('41-blocker-verifier-static-proof.png', 'Preserved genuine missing-method compatibility proof')
        ...
        for term in ('SCHEMA EVIDENCE', 'Primary / effective schema', 'ExampleSchema', 'What LedgerPreflight found', 'Why this matters', 'Recommended action'):
            assert term in terminal.display(), term
        if variant == 'blocked-no-tvu':
            terminal.capture('42-blocker-verifier-schema.png', 'Preserved human-readable schema evidence')
        terminal.action('Back')
        terminal.menu()
        if variant == 'blocked-tvu':
            ...
            for term in ('TVU EVIDENCE', '650', '449', '201', '201 supplied failures match'):
                assert term in terminal.display(), term
            terminal.capture('47-blocker-verifier-tvu-evidence.png', 'All supplied TVU failures correlate to the preserved blocker')
            terminal.action('Back')
            terminal.menu()
    ...
    if variant == 'blocked-no-tvu':
        terminal.capture('43-blocker-verifier-report-export.png', 'Full assessment with class, method and schema proof exported')
    terminal.action('Back')
    terminal.menu()
    ...
    for term in ('READY TO SHARE', 'Secrets redacted', 'Private keys excluded', 'Keystores excluded', 'Final package rescanned', 'SHA-256 generated'):
        assert term in terminal.display(), term
    if variant == 'blocked-no-tvu':
        terminal.capture('44-blocker-verifier-r3-support.png', 'Sanitized package includes exact verifier and schema proof')
    terminal.action('Back')
    terminal.menu()
    terminal.action('Exit')
    terminal.finish(checks.EXIT[variant])
    if variant == 'blocked-no-tvu':
        terminal.capture('45-blocker-verifier-exit.png', 'Clean terminal exit with cursor restored')
    results[terminal.name]['supportPackage'] = checks.validate_export(terminal.output, variant)
    results[terminal.name]['requiredClassAndInheritedHelperCompatible'] = True
    ...
    assert process.returncode == checks.EXIT[variant]
...
# A malformed required verifier class must still produce a real unresolved result.
unknown = ROOT / 'synthetic/blocker-verifier/unknown'
process = subprocess.run([LAUNCHER, 'assess', '--node', str(unknown / 'node'), '--upgrade-kit', str(unknown / 'kit'),
                          '--host-environment', str(unknown / 'host.json'), '--verifier-classpath', str(unknown / 'classpath.txt'),
                          '--network-mode', 'all-4.12', '--output', str(OUTPUT / 'unknown-non-tty'), '--json'],
                         capture_output=True, timeout=90)
assert process.returncode == checks.EXIT['unknown']
assert b'\x1b[' not in process.stdout
checks.report_proof(json.loads(process.stdout), 'unknown')
(TRANSCRIPTS / 'blocker-verifier-unknown-non-tty.json').write_bytes(process.stdout)
(TRANSCRIPTS / 'blocker-verifier-unknown-non-tty.stderr').write_bytes(process.stderr)
results['blocker-verifier-unknown-non-tty'] = {'exit': process.returncode, 'oneJsonDocument': True, 'noCursorControls': True, 'realIncompletePreserved': True}

(SHOTS / 'manifest-blocker-verifier.json').write_text(json.dumps(manifest, indent=2) + '\n')
(ROOT / 'blocker-verifier-terminal-validation.json').write_text(json.dumps(results, indent=2) + '\n')
