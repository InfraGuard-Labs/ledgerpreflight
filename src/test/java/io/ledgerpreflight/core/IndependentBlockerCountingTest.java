...
class IndependentBlockerCountingTest {
    ...
    @Test void unresolvedVerifierContextDoesNotCountKnownMethodTwice(){
        ...
        count(a,2);assertFalse(ProductView.result(a).contains("Compatibility analysis incomplete"));assertFalse(ProductView.result(a).contains("unresolved review"));assertTrue(Reports.html(a).contains("unknownContexts"));
    }
    @Test void largeResultsKeepAllBlockersInReportsAndSummarizeTheRest(){
        ...
        for(String title:List.of("CorDapp compatibility","TVU validation","Schema configuration","Target Java","Environment configuration","Upgrade preparation")){assertTrue(html.contains(title),title);}
        assertFalse(terminal.contains("6. "));assertTrue(terminal.contains("Additional issues are explained"));assertFalse(html.contains("Showing 3 of"));
    }
