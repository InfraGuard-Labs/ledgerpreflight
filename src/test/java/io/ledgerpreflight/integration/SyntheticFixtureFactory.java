...
public final class SyntheticFixtureFactory {
    ...
    public static byte[] emptyClass(String name){ClassWriter writer=new ClassWriter(0);writer.visit(Opcodes.V17,Opcodes.ACC_PUBLIC,name,null,"java/lang/Object",null);writer.visitEnd();return writer.toByteArray();}
    public static byte[] utility(boolean method){ClassWriter writer=new ClassWriter(0);writer.visit(Opcodes.V17,Opcodes.ACC_PUBLIC,OWNER,null,"java/lang/Object",null);if(method){MethodVisitor mv=writer.visitMethod(Opcodes.ACC_PUBLIC|Opcodes.ACC_STATIC,"sum",DESC,null,null);mv.visitCode();mv.visitInsn(Opcodes.ACONST_NULL);mv.visitInsn(Opcodes.ARETURN);mv.visitMaxs(1,1);mv.visitEnd();}writer.visitEnd();return writer.toByteArray();}
    public static byte[] contract(boolean call){ClassWriter writer=new ClassWriter(0);writer.visit(Opcodes.V17,Opcodes.ACC_PUBLIC,"example/SyntheticTokenContract",null,"java/lang/Object",null);MethodVisitor mv=writer.visitMethod(Opcodes.ACC_PUBLIC,"verifyConsume","()V",null,null);mv.visitCode();if(call){mv.visitInsn(Opcodes.ACONST_NULL);mv.visitMethodInsn(Opcodes.INVOKESTATIC,OWNER,"sum",DESC,false);mv.visitInsn(Opcodes.POP);}mv.visitInsn(Opcodes.RETURN);mv.visitMaxs(1,1);mv.visitEnd();writer.visitEnd();return writer.toByteArray();}
