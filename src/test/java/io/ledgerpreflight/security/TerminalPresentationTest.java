...
class TerminalPresentationTest {
    ...
    @Test void realReplayCliDefaultFitsOneScreenAndKeepsEvidenceInReport() throws Exception {
        ...
        assertTrue(output.contains("✕ NOT READY TO UPGRADE")); assertTrue(output.contains("3. Schema configuration")); assertFalse(output.contains("✓ PASS"));
    ...
    @Test void hundredsOfFindingsRemainShortAndEverySummaryLineFitsNarrowTerminal() {
        ...
        assertTrue(output.contains("1 blocker"));assertEquals(1,output.split("1. Upgrade preparation",-1).length-1);
