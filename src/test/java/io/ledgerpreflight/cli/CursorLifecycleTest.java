...
class CursorLifecycleTest {
    ...
    @Test void fatalFailureRestoresTerminalModeAndCursor(){
        StringWriter output=new StringWriter();List<String> modes=new ArrayList<>();
        Reader broken=new Reader(){public int read(char[] b,int o,int n){throw new OutOfMemoryError("synthetic fatal failure");}public void close(){}};
        SessionTerminal terminal=new SessionTerminal(broken,new PrintWriter(output),true,76){@Override protected String terminalMode(String...args){modes.add(String.join(" ",args));return "saved";}};
        assertThrows(OutOfMemoryError.class,()->terminal.choose("",List.of("Continue")));
        assertEquals("saved",modes.get(modes.size()-1));assertTrue(output.toString().endsWith("\r\n"));assertTrue(output.toString().contains("\u001b[?25h"));
    }
    @Test void fatalRawModeSetupFailureStillRestoresTerminal(){
        StringWriter output=new StringWriter();List<String> modes=new ArrayList<>();
        SessionTerminal terminal=new SessionTerminal(new StringReader(""),new PrintWriter(output),true,76){@Override protected String terminalMode(String...args){modes.add(args[0]);if(args[0].equals("-icanon"))throw new OutOfMemoryError("synthetic setup failure");return "saved";}};
        assertThrows(OutOfMemoryError.class,()->terminal.choose("",List.of("Continue")));
        assertEquals(List.of("-g","-icanon","saved"),modes);assertTrue(output.toString().contains("\u001b[?25h"));
    }
