...
public final class AssessmentService {
    ...
    public Assessment assess(Options options,java.util.function.Consumer<String> progress)throws IOException {
        ...
        if(tvuEvidence.schemaValidationFailure()){
            executedSchema=new TvuSchemaEvidence.Proof("FAILED",false,executedSchema.mismatch(),primarySchema,executedSchema.configuredValues(),executedSchema.sources(),List.of("Supplied TVU evidence reports schema validation failure"));
            schemaReadiness=new ConfigAnalyzer.TvuSchemaReadiness(configuredSchema.applicable(),false,"FAILED",configuredSchema.configurationSource(),configuredSchema.effectiveSchema(),configuredSchema.mappedProperty(),configuredSchema.configuredValue(),executedSchema.evidence());
            schemaSettings=new TreeMap<>(config.safeSettings());schemaSettings.put("tvuSchemaReadiness",schemaReadiness);
            config=new ConfigAnalyzer.ConfigEvidence(config.schema(),config.jdbcCurrentSchema(),config.hibernateDefaultSchema(),config.mixedCase(),config.contradictory(),config.postgresql(),config.issues(),config.sanitizedConfig(),Collections.unmodifiableMap(schemaSettings));
        }
        ...
        evidence.put("tvu-schema-execution",executedSchema);evidence.put("current-runtime-selection",active.evidence());evidence.put("tvu-evidence-supplied",!options.tvuResults().isEmpty());
