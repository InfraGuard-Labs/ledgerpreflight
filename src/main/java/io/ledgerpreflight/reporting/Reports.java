...
public final class Reports {
    ...
    public static String terminal(Assessment a,boolean verbose){
        if(!verbose)return conciseTerminal(a);
    ...
    public static String html(Assessment a){
        ...
        for(Finding f:priority.stream().limit(3).toList())h.append("<li><span class=\"rule\">").append(escape(f.id())).append("</span><h3>").append(escape(shortText(executiveTitle(f),145))).append("</h3><p>").append(escape(shortText(executiveWhy(f),190))).append("</p></li>");
        if(priority.isEmpty())h.append("<li><h3>").append(a.status().equals("READY TO UPGRADE")?"No blockers in the supplied evidence":a.status().equals("READY FOR TVU")?"Required TVU validation is outstanding":"Warning review is outstanding").append("</h3><p>").append(disposition).append("</p></li>");
        h.append("</ul>");
        if(priority.size()>3)h.append("<p class=\"review-note\">Showing 3 of ").append(priority.size()).append(" priority findings. <a href=\"#technical\">Review all technical findings below.</a></p>");
        ...
        List<String> actions=a.findings().stream().filter(f->!f.severity().equals("INFO")&&!f.status().equals("PASS")&&!f.id().equals("LP-TVU-002")).map(Reports::executiveAction).distinct().limit(2).toList();
