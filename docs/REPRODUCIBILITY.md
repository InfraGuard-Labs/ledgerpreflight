...
Release checks exercise static assessment and import existing synthetic TVU logs and error ZIPs. They verify that no public menu or command can launch TVU, while preserving import, correlation, reports, support sanitization and readiness checks. Test counts and platform outcomes are reported from the final release build; older candidate validation is not substituted for the shipped artifact.
