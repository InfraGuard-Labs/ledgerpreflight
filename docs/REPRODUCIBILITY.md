...
Hierarchy regressions use original generated facades over non-public superclass layers, different compatible current/target chains and a nested archive large enough to expose repeated enumeration work. No Kotlin or proprietary runtime binaries are used. `validate-hierarchy-package.sh` runs no-TVU, fully correlated TVU and compatible-only scenarios on each Ubuntu version; `validate-hierarchy-terminal.py` captures the actual packaged terminal; `validate-hierarchy.py` verifies A–U and declaration provenance. Existing execution-context, schema, TVU and memory regressions run alongside them. Consult the generated validation evidence for final test counts and artifact hashes.
...
Verifier class-presence and blocker regressions add identical class copies, genuine malformed required classes, exact missing methods, unproven/proven mixed-case schema configurations and both ready states. The validate-blocker-verifier-package.sh script exercises the actual bundled launcher across Ubuntu 18/20/22/24; validate-blocker-verifier-terminal.py captures actual terminal cell buffers; validate-blocker-verifier.py cross-checks A–V against generated reports, support archives and Docker/JUnit evidence. The fully supplied synthetic TVU run contains 650 processed, 449 passed and 201 failed transactions; all 201 detailed failures must retain exact compatibility correlation.

Guided TVU regressions generate executable synthetic validators that emit progress, spawn child processes and produce mixed diagnostic/binary error archives. The A–AH matrix covers explicit database confirmation, alternate configuration, source immutability, schema selection and loading, private Java, process cancellation, automatic import and correlation, support sanitization, repeated execution and readiness transitions. The validate-guided-tvu-package.sh, validate-guided-tvu-terminal.py and validate-guided-tvu.py scripts exercise the shipped launcher and retain actual terminal captures. Private real-environment acceptance remains an operator step before publication.
